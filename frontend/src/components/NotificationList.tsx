...
function InvitationNotification(props: {notification: Notification}) {
    ...
    const onClickAccept = () => {
        acceptInvitation.mutate({request: {invitationId: bytesToString(props.notification.data), userId: props.notification.receiverId, notificationId: props.notification.id}})
    }
    ...
    return (
        <Stack direction={"row"}>
            ...
            <Button disabled={acceptInvitation.isPending} onClick={onClickAccept}>{acceptInvitation.isPending ? <CircularProgress/> : <Check/>}</Button>
...
export default function InvitationNotificationList() {
    ...
    return (
        <Stack>
            {invitations.map(i => <InvitationNotification key={i.id} notification={i} />)}
        </Stack>
    )
}
...
export function PenaltyNotificationList() {
    ...
    return (
        <Stack>
            {penalties.map(p => <Typography key={p.id}>You have been penalized with {bytesToString(p.data)}</Typography>)}
        </Stack>
    )
}