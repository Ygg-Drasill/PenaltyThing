...
export default function InvitationNotificationList() {
  ...
  const invitations = appContext.notifications.data.filter(
    (n) => n.type == "INVITATION",
  );
...
export function PenaltyNotificationList() {
  ...
  const penalties = appContext.notifications.data.filter(
    (n) => n.type == "PENALTY",
  );
  ...
  return (
    <Stack>
      {penalties.map((p) => (
        <Typography key={p.id}>
          You have been penalized with {bytesToString(p.data)}
        </Typography>
      ))}
    </Stack>
  );
